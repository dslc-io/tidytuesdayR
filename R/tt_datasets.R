...
tt_available<-function(){
  tt_year<-tt_years()
  pastDatasets<-purrr::map(tt_year[-which.max(tt_year)],~tt_datasets(.x))%>%
...
tt_datasets<-function(year){
  if(missing(year)){
    url<-"https://github.com/rfordatascience/tidytuesday/blob/master/README.md"
    table<-1
  }else{
    url<-file.path("https://github.com/rfordatascience/tidytuesday/tree/master/data",year)
    table<-2
  }
  datasets<- url %>%
    xml2::read_html()%>%
