...
An R package that lets users download weekly datasets and their documentation from the TidyTuesday GitHub repository (<https://github.com/rfordatascience/tidytuesday>). Also includes a curation workflow for contributors who want to submit new datasets.
...
tidytuesdayR/
├── R/                          # R source code (one file ≈ one function or concern)
│   ├── aaa-*.R                 # Shared params, conditions, infrastructure (loaded first)
│   ├── tt_load.R / tt_load_gh.R / tt_download*.R  # Core data-loading functions
│   ├── tt_available.R / tt_datasets.R / tt_check_date.R  # Discovery & validation
│   ├── tt_clean.R / tt_intro.R / tt_meta.R /      # Data curation workflow
│   │   tt_save_dataset.R / tt_submit.R / tt_curate*.R
│   ├── use_tidytemplate.R / last_tuesday.R         # User helpers
│   ├── github_api.R / utils.R / utils-pipe.R       # Internal utilities
│   ├── tidytuesdayR-package.R                      # Package-level docs
│   └── zzz.R                                       # Package initialization
├── tests/testthat/             # testthat v3 test suite (25+ test files)
├── inst/templates/             # File templates (cleaning.R, meta.yaml, intro.md, etc.)
├── vignettes/                  # curating.Rmd — curation workflow guide
├── man/                        # roxygen2-generated documentation (29 .Rd files)
├── .github/
│   ├── ISSUE_TEMPLATE/         # GitHub issue templates
│   ├── skills/                 # Agent skill definitions
│   └── workflows/              # CI/CD (R-CMD-check, coverage)
...
| File | Purpose |
|------|---------|
| `R/tt_load.R` | Main user-facing function `tt_load()` — loads a dataset by date or year/week |
| `R/tt_load_gh.R` | `tt_load_gh()` — fetches file list and readme from TidyTuesday GitHub |
| `R/tt_download.R` / `R/tt_download_file.R` | Download all or individual dataset files; handles CSV, TSV, RDS, Excel, ZIP, VGZ |
| `R/tt_available.R` | `tt_available()` / `tt_datasets()` — dataset discovery functions |
| `R/tt_curate_data.R` + `tt_curate_utils.R` | Interactive step-by-step curation workflow |
| `R/tt_clean.R`, `tt_intro.R`, `tt_meta.R`, `tt_save_dataset.R`, `tt_submit.R` | Individual curation steps; `tt_submit()` opens a PR |
| `R/use_tidytemplate.R` | `use_tidytemplate()` — scaffolds an analysis `.Rmd` for users |
| `R/aaa-conditions.R` | Custom `.pkg_abort()` error conditions (uses `stbl`) |
...
| `inst/templates/` | Templates copied by `tt_clean()`, `tt_intro()`, `tt_meta()`, `use_tidytemplate()` |
