...
test_that("tt_make_html generates a properly formatted html doc", {
  tt_data<-structure(list(tt=list(readme="<p class=\"contents\">README contents</p>")))
...
test_that("`$` accesses the data from the tt_data object",{
  ...
  tt_data<-structure(
    list(data=list(value1=value1,
                   value2=value2)),class="tt_data")
...
test_that("print.tt_data lists the available datasets", {
  tt_data<-structure(
    list(data=list(value1="value1",
                   value2="value2"),
         tt=list(files=c("value1.csv","value2.csv"))),class="tt_data")
