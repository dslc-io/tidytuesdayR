...
test_that("tt_download_file messages for bad connectivity", {
  local_mocked_bindings(
    get_connectivity = function() FALSE,
    check_connectivity = function(rerun) NULL
  )
  expect_message(
    {expect_null(tt_download_file())},
    "Warning - No Internet Connectivity"
  )
})

test_that("tt_download_file returns NULL when no rate limit available", {
  local_mocked_bindings(
